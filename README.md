...
An external-storage source's volume is mounted into the Nextcloud container as well — NextExplorer's lands at `/mnt/nextexplorer` and FileBrowser Quantum's at `/mnt/filebrowser`, outside the `nextcloud` volume. NextExplorer's volume root is a list of locations rather than one tree, so it gets one `files_external` entry per location and none for the root: `Files` as `/NextExplorer`, and every other top-level directory as `/NextExplorer (<location>)` — its File Browser import lands in a location of its own, as can a service pointed at one, and its Add Location action adds more. The root is never exposed, because it also holds every account's private `_users` tree, hidden directories are skipped as they are in NextExplorer's own Locations list, and a directory `www-data` cannot open yet — one another service has just created under its own uid — is left out until it can. **Only a location that exists gets an entry, `Files` included**: NextExplorer creates it on its first start, and the dependency asks only that NextExplorer be installed, so one that has never run surfaces no `/NextExplorer` folder rather than a broken one. **That mount uses `idmap`** to remap the source's on-disk uid to `www-data`, so Nextcloud simply owns the tree: it reads, writes, and moves files with no permission machinery, and the files it creates land back on disk under the source's own uid so the source can still manage them.
...
- **Per-source scoping.** Each source is off, available to all users, or restricted to a chosen set, and every location of a source follows that one choice. An entry is created already restricted, never open first and narrowed afterwards. Clearing a source deletes its `files_external` entries; it does not delete any files.
- **Nextcloud's scoping is the only access control on the mounted files.** NextExplorer runs with per-account location access (`USER_VOLUMES`), but that list is NextExplorer's own and does not reach Nextcloud: every Nextcloud user the action selects sees every location, whichever NextExplorer accounts can see it.
- **Locations are read when the service starts, not while it runs.** A location that appears later — the File Browser import, a service given a location of its own, one made with NextExplorer's Add Location action — shows up in Files after the next restart; the `external-storage` oneshot runs one `find` per start and touches Nextcloud only when the set of locations has changed. A location that has gone loses its entry at that same point. **A renamed location is a new folder to Nextcloud**, which keys a local storage by its directory: favourites, tags and shares made under the old name do not follow it, whereas an entry deleted and created again on the same directory gets all of them back. What a re-created entry does lose is its own settings — a folder name or the sharing option an admin set on it — so a volume that cannot be read leaves every location's entry as it is, and the next start tries again.
- **A location's entry is matched by the directory it points at, not by its name**, so a folder renamed in Nextcloud's own External storage settings is kept rather than duplicated.
