...
docker-images/x86_64.tar: Dockerfile docker_entrypoint.sh nextcloud-init.sh nextcloud-run.sh
ifeq ($(ARCH),aarch64)
else
...
docker-images/aarch64.tar: Dockerfile docker_entrypoint.sh nextcloud-init.sh nextcloud-run.sh
ifeq ($(ARCH),x86_64)
else
	mkdir -p docker-images
